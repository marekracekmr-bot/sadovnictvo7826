const buriny = [

  // Sem budeš pridávať buriny

  {
  id: "amaranthus-retroflexus",
  latin: "Amaranthus retroflexus",
  name: "Amarant ohnutý",
  family: "Láskavcovité (Amaranthaceae)",
  category: "buriny",
  images: [
    "buriny/amaranthus-retroflexus1.jpg",
  ]
},

{
  id: "echinochloa-crus-galli",
  latin: "Echinochloa crus-galli",
  name: "Ježatka kuria",
  family: "Lipnicovité (Poaceae)",
  category: "buriny",
  images: [
    "buriny/echinochloa-crus-galli1.jpg",
    "buriny/echinochloa-crus-galli2.jpg",
  ]
},

{
  id: "oxalis-corniculata",
  latin: "Oxalis corniculata",
  name: "Kysličkovec rožkatý",
  family: "Kysličkovité (Oxalidaceae)",
  category: "buriny",
  images: [
    "buriny/oxalis-corniculata1.jpg",
    "buriny/oxalis-corniculata2.jpg",
  ]
},

{
  id: "chenopodium-hybridum",
  latin: "Chenopodium hybridum",
  name: "Mrlík hybridný",
  family: "Láskavcovité (Amaranthaceae)",
  category: "buriny",
  images: [
    "buriny/chenopodium-hybridum1.jpg",
    "buriny/chenopodium-hybridum2.jpg",
  ]
},

{
  id: "convolvulus-arvensis",
  latin: "Convolvulus arvensis",
  name: "Pupenec roľný",
  family: "Pupencovité (Convolvulaceae)",
  category: "buriny",
  images: [
    "buriny/convolvulus-arvensis1.jpg",
    "buriny/convolvulus-arvensis2.jpg",
    "buriny/convolvulus-arvensis3.jpg",
    "buriny/convolvulus-arvensis4.jpg",
    "buriny/convolvulus-arvensis5.jpg",
  ]
},

{
  id: "polygonum-aviculare",
  latin: "Polygonum aviculare",
  name: "Stavikrv vtáčí",
  family: "Stavikrvovité (Polygonaceae)",
  category: "buriny",
  images: [
    "buriny/polygonum-aviculare1.jpg",
    "buriny/polygonum-aviculare2.jpg",
  ]
},

{
  id: "plantago-lanceolata",
  latin: "Plantago lanceolata",
  name: "Skorocel kopijovitý",
  family: "Skorocelovité (Plantaginaceae)",
  category: "buriny",
  images: [
    "buriny/plantago-lanceolata1.jpg",
  ]
},

{
  id: "portulaca-oleracea",
  latin: "Portulaca oleracea",
  name: "Portulaka zeleninová",
  family: "Portulakovité (Portulacaceae)",
  category: "buriny",
  images: [
    "buriny/portulaca-oleracea1.jpg",
  ]
},

{
  id: "solanum-nigrum",
  latin: "Solanum nigrum",
  name: "Lúľok čierny",
  family: "Ľuľkovité (Solanaceae)",
  category: "buriny",
  images: [
    "buriny/solanum-nigrum1.jpg",
    "buriny/solanum-nigrum2.jpg",
    "buriny/solanum-nigrum3.jpg",
    "buriny/solanum-nigrum4.jpg",
  ]
},

{
  id: "panicum-effusum",
  latin: "Panicum effusum",
  name: "Proso rozložité",
  family: "Lipnicovité (Poaceae)",
  category: "buriny",
  images: [
    "buriny/panicum-effusum1.jpg",
    "buriny/panicum-effusum2.jpg",
    "buriny/panicum-effusum3.jpg",
    "buriny/panicum-effusum4.jpg",
  ]
},

{
  id: "setaria-pumila",
  latin: "Setaria pumila",
  name: "Mohar sivý",
  family: "Lipnicovité (Poaceae)",
  category: "buriny",
  images: [
    "buriny/setaria-pumila1.jpg",
    "buriny/setaria-pumila2.jpg",
  ]
},

{
  id: "phytolacca-americana",
  latin: "Phytolacca americana",
  name: "Ličidlo americké",
  family: "Ličidlovité (Phytolaccaceae)",
  category: "buriny",
  images: [
    "buriny/phytolacca-americana1.jpg",
    "buriny/phytolacca-americana2.jpg",
  ]
},

];

export default buriny;